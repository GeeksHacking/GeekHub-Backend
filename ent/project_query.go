...
func (pq *ProjectQuery) QueryTags() *TagQuery {
...
func (pq *ProjectQuery) QueryTickets() *TicketQuery {
	query := &TicketQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) QueryLanguages() *LanguageQuery {
	query := &LanguageQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withTickets:   pq.withTickets.Clone(),
		withLanguages: pq.withLanguages.Clone(),
...
func (pq *ProjectQuery) WithTags(opts ...func(*TagQuery)) *ProjectQuery {
	query := &TagQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProjectQuery) WithTickets(opts ...func(*TicketQuery)) *ProjectQuery {
	query := &TicketQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withTickets = query
	return pq
}
...
func (pq *ProjectQuery) WithLanguages(opts ...func(*LanguageQuery)) *ProjectQuery {
	query := &LanguageQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withLanguages = query
	return pq
}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context) ([]*Project, error) {
	...
	if query := pq.withUsers; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(project.UsersPrimaryKey[1], fks...))
			},
	...
	if query := pq.withTags; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(project.TagsPrimaryKey[0], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
	...
	if query := pq.withTickets; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		...
		query.Where(predicate.Ticket(func(s *sql.Selector) {
			s.Where(sql.InValues(project.TicketsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.project_tickets
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "project_tickets" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "project_tickets" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Tickets = append(node.Edges.Tickets, n)
		}
	}

	if query := pq.withLanguages; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Project, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Languages = []*Language{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Project)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(project.LanguagesPrimaryKey[0], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, pq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "languages": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "languages" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Languages = append(nodes[i].Edges.Languages, n)
			}
		}
	}
