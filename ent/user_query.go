...
type UserQuery struct {
	...
	withReportedTickets *TicketQuery
...
// QueryReportedTickets chains the current query on the "reported_tickets" edge.
func (uq *UserQuery) QueryReportedTickets() *TicketQuery {
...
func (uq *UserQuery) QueryAssignedTickets() *TicketQuery {
	query := &TicketQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withReportedTickets: uq.withReportedTickets.Clone(),
...
// WithReportedTickets tells the query-builder to eager-load the nodes that are connected to
// the "reported_tickets" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithReportedTickets(opts ...func(*TicketQuery)) *UserQuery {
	query := &TicketQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withReportedTickets = query
	return uq
}
...
func (uq *UserQuery) WithAssignedTickets(opts ...func(*TicketQuery)) *UserQuery {
	query := &TicketQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withAssignedTickets = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withProjects; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.ProjectsPrimaryKey[0], fks...))
			},
	...
	if query := uq.withReportedTickets; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ReportedTickets = []*Ticket{}
		}
		query.withFKs = true
		query.Where(predicate.Ticket(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ReportedTicketsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_reported_tickets
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_reported_tickets" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_reported_tickets" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ReportedTickets = append(node.Edges.ReportedTickets, n)
		}
	}

	if query := uq.withAssignedTickets; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.AssignedTickets = []*Ticket{}
		}
		query.withFKs = true
		query.Where(predicate.Ticket(func(s *sql.Selector) {
			s.Where(sql.InValues(user.AssignedTicketsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_assigned_tickets" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_assigned_tickets" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.AssignedTickets = append(node.Edges.AssignedTickets, n)
		}
	}
