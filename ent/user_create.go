...
func (uc *UserCreate) AddReportedTicketIDs(ids ...int) *UserCreate {
	uc.mutation.AddReportedTicketIDs(ids...)
	return uc
}

// AddReportedTickets adds the "reported_tickets" edges to the Ticket entity.
func (uc *UserCreate) AddReportedTickets(t ...*Ticket) *UserCreate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
	return uc.AddReportedTicketIDs(ids...)
}
...
func (uc *UserCreate) check() error {
	...
	if _, ok := uc.mutation.Auth0ID(); !ok {
		return &ValidationError{Name: "auth0_id", err: errors.New(`ent: missing required field "auth0_id"`)}
	}
	return nil
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.ReportedTicketsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.ReportedTicketsTable,
			Columns: []string{user.ReportedTicketsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
