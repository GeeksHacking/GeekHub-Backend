...
func (pc *ProjectCreate) AddTicketIDs(ids ...int) *ProjectCreate {
	pc.mutation.AddTicketIDs(ids...)
	return pc
}

// AddTickets adds the "tickets" edges to the Ticket entity.
func (pc *ProjectCreate) AddTickets(t ...*Ticket) *ProjectCreate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
	return pc.AddTicketIDs(ids...)
}

// AddLanguageIDs adds the "languages" edge to the Language entity by IDs.
func (pc *ProjectCreate) AddLanguageIDs(ids ...int) *ProjectCreate {
	pc.mutation.AddLanguageIDs(ids...)
	return pc
}

// AddLanguages adds the "languages" edges to the Language entity.
func (pc *ProjectCreate) AddLanguages(l ...*Language) *ProjectCreate {
	ids := make([]int, len(l))
	for i := range l {
		ids[i] = l[i].ID
	}
	return pc.AddLanguageIDs(ids...)
}
...
func (pc *ProjectCreate) createSpec() (*Project, *sqlgraph.CreateSpec) {
	...
	if nodes := pc.mutation.TicketsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
	...
	if nodes := pc.mutation.LanguagesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
